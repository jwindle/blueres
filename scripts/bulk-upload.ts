...
const COLLECTION = "org.blueres.resume.resume";
...
program
  .name("bulk-upload")
  .description("Upload resume JSON files from a directory to a PDS")
...
async function main() {
  ...
  if (opts.dryRun) {
    ...
    writes.forEach((w, i) => {
      ...
      console.log(`  ${i + 1}. ${record["meta"] ? (record["meta"] as Record<string, unknown>)["title"] : files[i]}`);
    });
